...
2. Start navigation stack with one of the profiles in `scripts/diagnostics/orca_hybrid_profiles.md`.
...
1. Copy `scripts/diagnostics/run_manifest_template.yaml` and fill the run metadata.
...
4. Generate scorecard:
   - `python3 scripts/diagnostics/orca_hybrid_scorecard.py --output-json logs/<robot>/scorecard.json`
5. Generate recommendation:
   - `python3 scripts/diagnostics/orca_promotion_decision.py logs/<robot>/scorecard.json`

## Emergency rollback

1. Stop launch.
2. Relaunch with rollback profile from `scripts/diagnostics/orca_hybrid_profiles.md`.
