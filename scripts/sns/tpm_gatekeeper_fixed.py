...
class TPMGatekeeper(Node):
    def __init__(self):
        ...
        # Create a timer to publish "READY" every 1 second
        self.auth_timer = self.create_timer(1.0, self.publish_status)
        self.get_logger().info("TPM Gatekeeper active. Broadcasting 'READY' heartbeat...")

    def publish_status(self):
        msg = String()
        msg.data = f"ROBOT_READY:{socket.gethostname()}"
        self.pub_status.publish(msg)

    def challenge_callback(self, msg):
        # BUG FIX 1: Cancel the heartbeat timer while processing the challenge
        ...
        # BUG FIX 3: Use a temp directory instead of the current working directory.
        ...
        try:
            ...
            # BUG FIX 5: Added timeout=10 so a hung openssl call doesn't freeze the node.
            subprocess.run(
            ...
            # 3. Sign the HASH, not the file (this is more reliable across platforms)
            cmd = [
                "tpm2_sign",
                "-c", "0x81010002",
                "-g", "sha256",
            ...
            # BUG FIX 5: Added timeout=15 for the TPM call — hardware ops can be slow
            ...
            # BUG FIX 4: Publish an error so the challenger knows something went wrong
            ...
            # BUG FIX 4: Publish an error response on any failure so the authenticator
            # can react (retry, alarm, etc.) rather than hanging indefinitely.
            self._publish_error(f"AUTH_ERROR: {str(e)}")

        finally:
            # BUG FIX 2: Always clean up temp files, even if signing failed.
