...
[Unit]
Description=Boot WiFi Connection (SNS, then Starlink, then Azure)
...
echo ""
echo "Prereq: only one netplan file should configure wlan0. If \`switch_wifi.sh\` applies SNS/Starlink/Azure,"
