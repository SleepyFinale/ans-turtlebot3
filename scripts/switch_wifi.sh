#!/bin/bash
#
# Switch Raspberry Pi WiFi between GCRI_LAB (lab) and Azure (mobile hotspot).
# Uses netplan only; no NetworkManager required.
#
# Usage:
#   sudo ./scripts/switch_wifi.sh lab    # GCRI_LAB with static IP 192.168.50.219
...
#
# Static IP on GCRI_LAB is chosen by current user (pinky -> .219, blinky -> .193).
# When run with sudo we use SUDO_USER so "pinky" user gets .219. Override: $0 lab blinky or ROBOT_NAME=blinky.
#
...
# Set LAB_STATIC_IP from robot name (pinky -> LAB_IP_PINKY, blinky -> LAB_IP_BLINKY). Exits if unknown.
set_lab_static_ip() {
  local robot
...
usage() {
  ...
  echo "  lab [pinky|blinky] - connect to GCRI_LAB (static IP by robot)"
...
case "${1:-}" in
  lab)
    ...
    netplan_apply_quiet
    echo "Switched to GCRI_LAB (static IP $LAB_STATIC_IP)."
    ;;
  azure)
    if [ "$(id -u)" -ne 0 ]; then
